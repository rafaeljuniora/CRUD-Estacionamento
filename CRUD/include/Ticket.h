...
int funcTicket() {
    ...
    if (file == NULL) {
        ...
        if (cadastroVeiculo[i].id_veiculo == cadastroVaga[i].id_vaga &&
            ...
            fprintf(file, "\n===============TOTAL===============\n");
            if(cadastroVaga[i].ativo==1 && cadastroEntradaSaida[i].ativo==1){valor_hora = valor_vaga * cadastroEntradaSaida[i].hora_total;
            }else{
                valor_hora=0;
            }
            
            if(Carrinho[i].ativo==1){valor_carrinho = (Carrinho[i].lavagem * 50) + (Carrinho[i].polimento * 100) + (Carrinho[i].troca_oleo * 100);
            }else{
                valor_carrinho=0;
            }
